#pragma once

namespace Turbo
{
	namespace ELinearColor
	{
		const glm::vec4 kTransparent {0.f, 0.f, 0.f, 0.f};

		const glm::vec4 kRed {1.f, 0.f, 0.f, 1.f};
		const glm::vec4 kGreen {0.f, 1.f, 0.f, 1.f};
		const glm::vec4 kBlue {0.f, 0.f, 1.f, 1.f};

		const glm::vec4 kCyan {0.f, 1.f, 1.f, 1.f};
		const glm::vec4 kMagenta {1.f, 0.f, 1.f, 1.f};
		const glm::vec4 kYellow {1.f, 1.f, 0.f, 1.f};

		const glm::vec4 kWhite {1.f, 1.f, 1.f, 1.f};
		const glm::vec4 kBlack {0.f, 0.f, 0.f, 1.f};
	};
}